...
class FlutterBluetoothClassic {
  ...
  FlutterBluetoothClassic._() {
    ...
    _stateChannel.receiveBroadcastStream().listen((dynamic event) {
      // FIX: Handle Map<Object?, Object?> safely
      final Map<String, dynamic> eventMap = Map<String, dynamic>.from(event as Map);
    ...
    _connectionChannel.receiveBroadcastStream().listen((dynamic event) {
      _connectionStreamController.add(BluetoothConnectionState.fromMap(Map<String, dynamic>.from(event as Map)));
    });

    // Listen for data received
    _dataChannel.receiveBroadcastStream().listen((dynamic event) {
      _dataStreamController.add(BluetoothData.fromMap(Map<String, dynamic>.from(event as Map)));
    });
  }
